...
type tfBackend struct {
	*framework.Backend
	...
	// interleave. Config writes and deletes also take it so they cannot clobber
	// a rotation's read-modify-write of the config entry. It must be distinct
	// from lock, which guards the client cache and is taken by getClient and
	// reset (both called during rotation).
